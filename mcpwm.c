...
//static volatile int ADC_V_L1;
//static volatile int ADC_V_L2;
//static volatile int ADC_V_L3;
...
filter_create_fir_lowpass((float*)kv_fir_coeffs, KV_FIR_FCUT, KV_FIR_TAPS_BITS,
						1);

// Create amplitude FIR filter
filter_create_fir_lowpass((float*)amp_fir_coeffs, AMP_FIR_FCUT,
AMP_FIR_TAPS_BITS,
						1);

// Create current FIR filter
filter_create_fir_lowpass((float*)current_fir_coeffs, CURR_FIR_FCUT,
CURR_FIR_TAPS_BITS,
						1);
...
//ADC_ExternalTrigInjectedConvConfig(ADC3, ADC_ExternalTrigInjecConv_T2_CC1);
...
uint16_t PrescalerValue = (uint16_t)(
		(SYSTEM_CORE_CLOCK / 2) / MCPWM_RPM_TIMER_FREQ) - 1;
...
chThdCreateStatic(timer_thread_wa, sizeof(timer_thread_wa), NORMALPRIO,
				timer_thread, NULL);
chThdCreateStatic(rpm_thread_wa, sizeof(rpm_thread_wa), NORMALPRIO, rpm_thread,
				NULL);
...
int curr0_offset2 = curr2_sum / curr_start_samples;
int curr1_offset2 = curr3_sum / curr_start_samples;
mcpwm_set_current_dccal2(curr0_offset2, curr1_offset2);
...
utils_truncate_number(&current, -fabsf(conf->l_current_min),
					fabsf(conf->l_current_min));
...
float value = filter_run_fir_iteration((float*)kv_fir_samples,
								(float*)kv_fir_coeffs, KV_FIR_TAPS_BITS,
								kv_fir_index);
...
if (conf->motor_type == MOTOR_TYPE_DC) {
	...
	if (IS_DETECTING() || conf->pwm_mode == PWM_MODE_BIPOLAR) {
		...
		switching_frequency_now = (float)conf->m_bldc_f_sw_min
				* (1.0 - fabsf(dutyCycle))
...
if (conf->motor_type
		== MOTOR_TYPE_BLDC&& conf->pwm_mode == PWM_MODE_BIPOLAR && !IS_DETECTING()) {
...
d_term = (error - prev_error) * (conf->s_pid_kd / MCPWM_PID_TIME_K)
		* (1.0 / 20.0);
...
for (;;) {
	...
	rpm_dep.cycle_int_limit_running = rpm_dep.cycle_int_limit
			+ (float)ADC_Value[ADC_IND_VIN_SENS] * conf->sl_bemf_coupling_k
					/ (rpm_abs > conf->sl_min_erpm ? 	rpm_abs :
												conf->sl_min_erpm);
	rpm_dep.cycle_int_limit_running = utils_map(
			rpm_abs, 0, conf->sl_cycle_int_rpm_br,
	...
	rpm_dep.comm_time_sum_min_rpm = conf->m_bldc_f_sw_max
			/ ((conf->sl_min_erpm / 60.0) * 6.0);
...
for (;;) {
	...
	if (state == MC_STATE_OFF) {
		...
		utils_truncate_number((float*)&dutycycle_now, -conf->l_max_duty,
							conf->l_max_duty);
	}
	else {
		tachometer_for_direction = 0;
...
int curr0_2 = ADC_GetInjectedConversionValue(ADC2, ADC_InjectedChannel_1);
int curr1_2 = ADC_GetInjectedConversionValue(ADC2, ADC_InjectedChannel_2);

mcpwm_adc_inj_int_handler2(curr0_2, curr1_2);

// ADC_V_L1 = ADC_GetInjectedConversionValue(ADC3, ADC_InjectedChannel_1);
//ADC_V_L2 = ADC_GetInjectedConversionValue(ADC3, ADC_InjectedChannel_2);
//ADC_V_L3 = ADC_GetInjectedConversionValue(ADC3, ADC_InjectedChannel_3);
	...
	if (detect_now == 4) {
		...
		if (detect_step > 0) {
			mcpwm_detect_currents_diff[detect_step] =
					mcpwm_detect_currents[detect_step - 1]
							- mcpwm_detect_currents[detect_step];
		}
		else {
			mcpwm_detect_currents_diff[detect_step] =
					mcpwm_detect_currents[5]
							- mcpwm_detect_currents[detect_step];
		}
		...
		mcpwm_detect_currents_avg[detect_step] +=
				mcpwm_detect_currents[detect_step];
...
if (fabsf(last_current_sample) > (conf->l_abs_current_max * 1.2)) {
	last_current_sample = SIGN(last_current_sample) * conf->l_abs_current_max
			* 1.2;
}
...
if (state == MC_STATE_RUNNING && has_commutated) {
	...
	float ramp_step = conf->m_duty_ramp_step
			/ (switching_frequency_now / 1000.0);
	...
	if (control_mode == CONTROL_MODE_CURRENT
...
#endif
		// Compute error
		const float error = current_set
				- (direction ? current_nofilter : -current_nofilter);
		...
		utils_truncate_number(&step, -conf->cc_ramp_step_max,
							conf->cc_ramp_step_max);
		...
		if (fabsf(dutycycle_now_tmp) < start_boost) {
			utils_step_towards(
					&dutycycle_now_tmp,
					current_set > 0.0 ? start_boost : -start_boost,
					ramp_step);
		}
		else {
		...
		// Upper truncation
		utils_truncate_number((float*)&dutycycle_now_tmp, -conf->l_max_duty,
							conf->l_max_duty);
		...
		dutycycle_set =
				dutycycle_now_tmp >= 0.0 ? 	conf->l_min_duty :
										-conf->l_min_duty;
	}
		...
		if (fabsf(dutycycle_now_tmp) < conf->l_min_duty) {
			if (fabsf(rpm_now) < conf->l_max_erpm_fbrake_cc) {
				...
				dutycycle_now_tmp = SIGN(dutycycle_now_tmp)
						* conf->l_min_duty;
				dutycycle_set = dutycycle_now_tmp;
			}
		}
		...
		utils_step_towards((float*)&dutycycle_now_tmp, dutycycle_set,
						ramp_step);
	}
	...
	if (current_nofilter > conf->lo_current_max) {
		utils_step_towards(
				...
				0.0,
				ramp_step_no_lim
						* fabsf(current_nofilter - conf->lo_current_max)
		...
		utils_step_towards(
				...
				ramp_step_no_lim
						* fabsf(current_nofilter - conf->lo_current_min)
						* conf->m_current_backoff_gain);
		limit_delay = 1;
	}
		...
		utils_step_towards(
				...
				0.0,
				ramp_step_no_lim
						* fabsf(current_in_nofilter
								- conf->lo_in_current_max)
						* conf->m_current_backoff_gain);
		limit_delay = 1;
	}
	...
	else if (dutycycle_now <= -conf->l_min_duty
			&& rpm > conf->l_max_erpm_fbrake) {
		dutycycle_now = conf->l_min_duty;
	}
...
//volatile uint32_t duty2 = mcpwm_get_timer_duty2();
volatile uint32_t top = timer_tmp->top;
unsigned int duty2 = mcpwm_get_timer_duty2();
...
if (IS_DETECTING()) {
	...
	curr2_sample = duty2 + (top - duty2) / 2;
}
else {
	// Voltage samples
	if (duty > duty2) {
...
timer_tmp->curr2_sample = curr1_sample;
timer_tmp->val_sample = val_sample;
//mcpwm_set_timer_curr1_sample2(curr2_sample);
//timer_tmp->curr3_sample = curr2_sample;
}
...
if (tacho_diff != 0) {
	...
	rpm_dep.time_at_comm += TIM13->CNT;
	TIM13->CNT = 0;
}
...
if (!timer_struct.updated && TIM1->CNT > 10 && TIM1->CNT < (TIM1->ARR - 500)) {
	...
	TIM2->CCR2 = timer_struct.val_sample / 2;
