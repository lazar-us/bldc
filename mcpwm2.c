...
static volatile mc_rpm_dep_struct rpm_dep2;
...
// Global variables
volatile float mcpwm_detect_currents2[6];
volatile float mcpwm_detect_voltages2[6];
volatile float mcpwm_detect_currents_diff2[6];
volatile int mcpwm_vzero2;
...
static void update_rpm_tacho2(void);
...
// Threads
static THD_WORKING_AREA(timer_thread2_wa, 2048);
static THD_FUNCTION(timer_thread2, arg);
static THD_WORKING_AREA(rpm_thread2_wa, 1024);
static THD_FUNCTION(rpm_thread2, arg);
...
void mcpwm_init2(volatile mc_configuration *configuration) {
	...
	has_commutated = 0;
	memset((void*)&rpm_dep2, 0, sizeof(rpm_dep2));
	...
	mcpwm_init_hall_table2((int8_t*)conf->hall_table);
    ...
    timer_tmp.top = TIM8->ARR;
    timer_tmp.duty = TIM8->ARR / 2;
    //TIM2->CCR2 = 5;
    ...
    rpm_thd_stop = false;
    chThdCreateStatic(timer_thread2_wa, sizeof(timer_thread2_wa), NORMALPRIO, timer_thread2, NULL);
    chThdCreateStatic(rpm_thread2_wa, sizeof(rpm_thread2_wa), NORMALPRIO, rpm_thread2, NULL);
...
float mcpwm_get_tot_current_directional2(void) {
	const float retval = mcpwm_get_tot_current2();
...
float mcpwm_get_tot_current_directional_filtered2(void) {
	const float retval = mcpwm_get_tot_current_filtered2();
	return dutycycle_now > 0.0 ? retval : -retval;
}
...
float mcpwm_get_tot_current_in2(void) {
	return mcpwm_get_tot_current2() * fabsf(dutycycle_now);
}
...
float mcpwm_get_tot_current_in_filtered2(void) {
	return mcpwm_get_tot_current_filtered2() * fabsf(dutycycle_now);
}
		...
		if (sensorless_now) {
			if (state != MC_STATE_RUNNING) {
				if (state == MC_STATE_OFF) {
					...
					if (fabsf(rpm_now) < conf->sl_min_erpm && mcpwm_get_tot_current_filtered2() < conf->sl_max_fullbreak_current_dir_change) {
						state = MC_STATE_RUNNING;
						commutate(1);
					}
				}
			...
			if (state != MC_STATE_RUNNING) {
				state = MC_STATE_RUNNING;
				comm_step = mcpwm_read_hall_phase2();
				set_next_comm_step(comm_step);
				commutate(1);
			}
		}
...
static void set_duty_cycle_hw(float dutyCycle) {
	...
	timer_tmp.top = mcpwm_get_timer_top();
...
static void run_pid_control_speed(void) {
	...
	const float rpm = mcpwm_get_rpm2();
...
static THD_FUNCTION(rpm_thread2, arg) {
	...
	for (;;) {
		...
		if (rpm_dep2.comms != 0) {
			utils_sys_lock_cnt();
			const float comms = (float)rpm_dep2.comms;
			const float time_at_comm = (float)rpm_dep2.time_at_comm;
			rpm_dep2.comms = 0;
			rpm_dep2.time_at_comm = 0;
			utils_sys_unlock_cnt();

			rpm_now = (comms * MCPWM_RPM_TIMER_FREQ * 60.0) / (time_at_comm * 6.0);
		} else {
			// In case we have slowed down
			float rpm_tmp = (MCPWM_RPM_TIMER_FREQ * 60.0) / ((float) TIM14 ->CNT * 6.0);
		...
		rpm_dep2.cycle_int_limit = conf->sl_cycle_int_limit;
		rpm_dep2.cycle_int_limit_running = rpm_dep2.cycle_int_limit + (float)ADC_Value[ADC_IND_VIN_SENS] *
				conf->sl_bemf_coupling_k / (rpm_abs > conf->sl_min_erpm ? rpm_abs : conf->sl_min_erpm);
		rpm_dep2.cycle_int_limit_running = utils_map(rpm_abs, 0,
				conf->sl_cycle_int_rpm_br, rpm_dep2.cycle_int_limit_running,
				rpm_dep2.cycle_int_limit_running * conf->sl_phase_advance_at_br);
		rpm_dep2.cycle_int_limit_max = rpm_dep2.cycle_int_limit + (float)ADC_Value[ADC_IND_VIN_SENS] *
				conf->sl_bemf_coupling_k / conf->sl_min_erpm_cycle_int_limit;

		if (rpm_dep2.cycle_int_limit_running < 1.0) {
			rpm_dep2.cycle_int_limit_running = 1.0;
		}

		if (rpm_dep2.cycle_int_limit_running > rpm_dep2.cycle_int_limit_max) {
			rpm_dep2.cycle_int_limit_running = rpm_dep2.cycle_int_limit_max;
		}

		rpm_dep2.comm_time_sum = conf->m_bldc_f_sw_max / ((rpm_abs / 60.0) * 6.0);
		rpm_dep2.comm_time_sum_min_rpm = conf->m_bldc_f_sw_max / ((conf->sl_min_erpm / 60.0) * 6.0);
...
static THD_FUNCTION(timer_thread2, arg) {
	...
	chRegSetThreadName("mcpwm timer2");
	...
	for(;;) {
		...
		if (cnt_tmp >= 10) {
			cnt_tmp = 0;
			if (state == MC_STATE_RUNNING) {
				filter_add_sample((float*)kv_fir_samples, mcpwm_get_kv2(),
...
void mcpwm_adc_inj_int_handler2(int curr0, int curr1) {
	...
	if (conf->motor_type == MOTOR_TYPE_DC) {
		if (direction) {
			curr_tot_sample = -(float)(ADC_Value[ADC_IND_CURR4] - curr1_offset);
		} else {
			curr_tot_sample = -(float)(ADC_Value[ADC_IND_CURR3] - curr0_offset);
		}
	} else {
		...
		if (detect_now == 4) {
			...
			if (a > b) {
				mcpwm_detect_currents2[detect_step] = a;
			} else {
				mcpwm_detect_currents2[detect_step] = b;
			}

			if (detect_step > 0) {
				mcpwm_detect_currents_diff2[detect_step] =
						mcpwm_detect_currents2[detect_step - 1] - mcpwm_detect_currents2[detect_step];
			} else {
				mcpwm_detect_currents_diff2[detect_step] =
						mcpwm_detect_currents2[5] - mcpwm_detect_currents2[detect_step];
			}
			...
			case 1:
			case 4:
				mcpwm_detect_voltages2[detect_step] = ADC_V_L4 - vzero;
				break;

			case 2:
			case 5:
				mcpwm_detect_voltages2[detect_step] = ADC_V_L5 - vzero;
				break;

			case 3:
			case 6:
				mcpwm_detect_voltages2[detect_step] = ADC_V_L6 - vzero;
				break;
			...
			mcpwm_detect_currents_avg[detect_step] += mcpwm_detect_currents2[detect_step];
...
void mcpwm_adc_int_handler2(void) {
	...
	if (conf->motor_type == MOTOR_TYPE_BLDC) {
		...
		if (has_commutated && fabsf(dutycycle_now) > 0.2) {
			mcpwm_vzero2 = ADC_V_ZERO;
		} else {
			mcpwm_vzero2 = (ADC_V_L4 + ADC_V_L5 + ADC_V_L6) / 3;
		}

		if (direction) {
			ph1 = ADC_V_L4 - mcpwm_vzero2;
			ph2 = ADC_V_L5 - mcpwm_vzero2;
			ph3 = ADC_V_L6 - mcpwm_vzero2;
			ph1_raw = ADC_V_L4;
			ph2_raw = ADC_V_L5;
			ph3_raw = ADC_V_L6;
		} else {
			ph1 = ADC_V_L4 - mcpwm_vzero2;
			ph2 = ADC_V_L6 - mcpwm_vzero2;
			ph3 = ADC_V_L5 - mcpwm_vzero2;
			ph1_raw = ADC_V_L4;
			ph2_raw = ADC_V_L6;
			ph3_raw = ADC_V_L5;
		}
		...
		if (sensorless_now) {
			...
			if (pwm_cycles_sum >= rpm_dep2.comm_time_sum_min_rpm) {
				if (state == MC_STATE_RUNNING) {
			...
			if ((state == MC_STATE_RUNNING && pwm_cycles >= 2) || state == MC_STATE_OFF) {
				...
				if (conf->comm_mode == COMM_MODE_INTEGRATE) {
					float limit;
					if (has_commutated) {
						limit = rpm_dep2.cycle_int_limit_running * (0.0005 * VDIV_CORR);
					} else {
						limit = rpm_dep2.cycle_int_limit * (0.0005 * VDIV_CORR);
					}

					if (cycle_integrator >= (rpm_dep2.cycle_int_limit_max * (0.0005 * VDIV_CORR)) ||
					...
					if (v_diff > 0) {
						...
						if (cycle_sum >= utils_map(fabsf(rpm_now), 0,
								conf->sl_cycle_int_rpm_br, rpm_dep2.comm_time_sum / 2.0,
								(rpm_dep2.comm_time_sum / 2.0) * conf->sl_phase_advance_at_br)) {
							commutate(1);
			...
			const int hall_phase = mcpwm_read_hall_phase2();
	...
	const float current_nofilter = mcpwm_get_tot_current2();
...
static int read_hall(void) {
	return READ_HALL4() | (READ_HALL5() << 1) | (READ_HALL6() << 2);
}
...
static void update_rpm_tacho2(void) {
	...
	if (tacho_diff != 0) {
		rpm_dep2.comms += tacho_diff;
		rpm_dep2.time_at_comm +=  TIM14->CNT;
		TIM14->CNT = 0;
	}
...
static void update_sensor_mode(void) {
	if (conf->sensor_mode == SENSOR_MODE_SENSORLESS ||
			(conf->sensor_mode == SENSOR_MODE_HYBRID &&
					fabsf(mcpwm_get_rpm2()) > conf->hall_sl_erpm)) {
		sensorless_now = true;
	} else {
...
static void set_next_comm_step(int next_step) {
	if (conf->motor_type == MOTOR_TYPE_DC) {
		...
		if (direction) {
			...
			// -
			TIM_SelectOCxM(TIM8, TIM_Channel_3, TIM_OCMode_Inactive);
			TIM_CCxCmd(TIM8, TIM_Channel_3, TIM_CCx_Enable);
			TIM_CCxNCmd(TIM8, TIM_Channel_3, TIM_CCxN_Enable);
		} else {
			// +
	...
	if (next_step == 1) {
		if (direction) {
			...
			// +
			TIM_SelectOCxM(TIM8, TIM_Channel_2, positive_oc_mode);
			TIM_CCxCmd(TIM8, TIM_Channel_2, positive_highside);
			TIM_CCxNCmd(TIM8, TIM_Channel_2, positive_lowside);

			// -
			TIM_SelectOCxM(TIM8, TIM_Channel_3, negative_oc_mode);
			TIM_CCxCmd(TIM8, TIM_Channel_3, negative_highside);
			TIM_CCxNCmd(TIM8, TIM_Channel_3, negative_lowside);
		} else {
#ifdef HW_HAS_DRV8313
			DISABLE_BR1();
			ENABLE_BR3();
			...
			// +
			TIM_SelectOCxM(TIM8, TIM_Channel_3, positive_oc_mode);
			TIM_CCxCmd(TIM8, TIM_Channel_3, positive_highside);
			TIM_CCxNCmd(TIM8, TIM_Channel_3, positive_lowside);

			// -
			TIM_SelectOCxM(TIM8, TIM_Channel_2, negative_oc_mode);
			TIM_CCxCmd(TIM8, TIM_Channel_2, negative_highside);
			TIM_CCxNCmd(TIM8, TIM_Channel_2, negative_lowside);
		}
	} else if (next_step == 2) {
		if (direction) {
#ifdef HW_HAS_DRV8313
			...
			// +
			TIM_SelectOCxM(TIM8, TIM_Channel_1, positive_oc_mode);
			TIM_CCxCmd(TIM8, TIM_Channel_1, positive_highside);
			TIM_CCxNCmd(TIM8, TIM_Channel_1, positive_lowside);
		...
		if (direction) {
			...
			// -
			TIM_SelectOCxM(TIM8, TIM_Channel_1, negative_oc_mode);
			TIM_CCxCmd(TIM8, TIM_Channel_1, negative_highside);
			TIM_CCxNCmd(TIM8, TIM_Channel_1, negative_lowside);
		} else {
#ifdef HW_HAS_DRV8313
			DISABLE_BR3();
			ENABLE_BR2();
