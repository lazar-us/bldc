	...
	for(;;) {
		...
		case ADC_CTRL_TYPE_DUTY_REV_BUTTON:
			...
			if (!(ms_without_power < MIN_MS_WITHOUT_POWER && config.safe_start)) {
				mc_interface_set_duty(utils_map(pwr, -1.0, 1.0, -mcconf->l_max_duty, mcconf->l_max_duty),utils_map(pwr, -1.0, 1.0, -mcconf->l_max_duty, mcconf->l_max_duty));
				send_duty = true;
			}
			break;
		...
		case ADC_CTRL_TYPE_PID_REV_BUTTON:
			...
			if (!(ms_without_power < MIN_MS_WITHOUT_POWER && config.safe_start)) {
				...
				mc_interface_set_pid_speed(speed,speed);
				send_duty = true;
			}
