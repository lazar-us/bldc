  ...
  for (;;) {
    ...
    if (fault != FAULT_CODE_NONE) {
      for (int i = 0; i < (int)fault; i++) {
        ...
        ledpwm_set_intensity(SWITCH_LED_1, 0.0);
        chThdSleepMilliseconds(250);
      }
...
int main(void) {
  ...
  mc_configuration mcconf2;
  conf_general_read_mc_configuration(&mcconf, &mcconf2);
  mc_interface_init(&mcconf, &mcconf2);
