      ...
      if (rxmsg.IDE == CAN_IDE_EXT) {
        ...
        if (id == 255 || id == app_get_configuration()->controller_id) {
          ...
          case CAN_PACKET_SET_DUTY:
            ind = 0;
            cmd1 = (float)buffer_get_int32(rxmsg.data8, &ind) / 100000.0;
            mc_interface_set_duty(cmd1,cmd1);
            timeout_reset();
            break;
          ...
          case CAN_PACKET_SET_CURRENT_BRAKE:
            ind = 0;
            cmd1 = (float)buffer_get_int32(rxmsg.data8, &ind) / 1000.0;
            mc_interface_set_brake_current(cmd1,cmd1);
            timeout_reset();
            break;

          case CAN_PACKET_SET_RPM:
            ind = 0;
            cmd1 = (float)buffer_get_int32(rxmsg.data8, &ind);
            mc_interface_set_pid_speed(cmd1,cmd1);
            timeout_reset();
            break;

          case CAN_PACKET_SET_POS:
            ind = 0;
            cmd1 = (float)buffer_get_int32(rxmsg.data8, &ind) / 1000000.0;
            mc_interface_set_pid_pos(cmd1,cmd1);
            timeout_reset();
            break;
